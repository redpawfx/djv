...
Movie::_Movie::~_Movie()
{
  if (_qt_audio) qt::MovieAudioExtractionEnd(_qt_audio);
  if (_qt_movie) qt::DisposeMovie(_qt_movie);
  if (_qt_gworld) qt::DisposeGWorld(_qt_gworld);
...
void Movie::_Movie::init(const String & in) throw (Error)
{
  ...
  if (err != qt::noErr)
    throw Error(string::Format("init = %%").arg(err));
#endif
 
  long version = 0;
  err = qt::Gestalt(qt::gestaltQuickTime, &version);
  if (err != qt::noErr)
    throw Error(string::Format("version = %%").arg(err));
  
  err = qt::EnterMovies();
  if (err != qt::noErr)
    throw Error( string::Format("enter = %%").arg(err));

  // Open.

  CFStringRef file(file::path_absolute(in));
  ...
  if (err != qt::noErr)
    throw Error(string::Format("open = %%").arg(err));
  ...
  if (err != qt::noErr)
    throw Error(string::Format("MovieAudioExtractionBegin = %%").arg(err));
  ...
  if (err != qt::noErr)
    throw Error(string::Format("MovieAudioExtractionGetPropertyInfo = %%").
      arg(err));
  ...
  /*struct Audio_Channel
  {
    ...
    void init(qt::MovieAudioExtractionRef in) throw (Error)
    {
      ...
      if (err != qt::noErr)
        throw Error(string::Format("MovieAudioExtractionGetProperty = %%").
          arg(err));
    }
  ...
  if (err != qt::noErr)
    throw Error(string::Format("MovieAudioExtractionGetProperty = %%").arg(err));
  ...
  if (err != qt::noErr)
    throw Error(string::Format("MovieAudioExtractionSetProperty = %%").arg(err));
  ...
  if (err != qt::noErr)
    throw Error(string::Format("gworld = %%").arg(err));

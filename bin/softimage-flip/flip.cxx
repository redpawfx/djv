...
void processOld( int argc, char **argv )
{
  ...
  if (argc >= 2 && argc < 6)
    {
    //filename is only useful information provided so launch djv_view with the single filename argument;
    command += " " + filename;
  ...
  if (argc > 6)
    ...
    while (i < argc)
      ...
      if (checkOption == "-p")
        {
        //it's an option that provides the padstring argument - so let's read the next one
        padString = argv[i];
        i++;
        }
      }
  ...
  int firstHash = padString.find_first_of("#");
  int lastHash = padString.find_last_of("#");
  int padding = 1;
  ...
  frameRange << setfill('0') << setw(padding) << start << "-" << setw(padding) << end;
...
void processNew( int argc, char **argv )
{
  ...
  int firstHash = filename.find_first_of("#");
  int lastHash = filename.find_last_of("#");
  int padding = 1;
...
int main( int argc, char **argv )
{
  ...
  if (argc == 1)
    {
    //there were no argument supplied so just launch djv_view without any arguments
    system("djv_view");
    return 0;
    }

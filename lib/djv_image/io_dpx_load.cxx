...
List<String> Load::option_value(const String & in) const
{
  ...
  if (string::find_no_case(in, label_load_option(), &index))
    switch (index)
    {
      case LOAD_COLOR_PROFILE: out = io_cineon::label_color_profile(); break;
      case LOAD_FILM_PRINT: out += io_cineon::label_film_print_load; break;
      case LOAD_CONVERT: out = io_cineon::label_convert(); break;
    }
...
const Image * Load::get(const io::Frame_Info & frame) throw (Error)
{
  ...
  if (
    ...
    (io_cineon::COLOR_PROFILE_AUTO == _option.color_profile && _film_print)
  ) {
  ...
  if (! frame.proxy && io_cineon::CONVERT_NONE == _option.convert)
  {
    _image.set(info, p);
  }
  else
  {
    ...
    {
      case io_cineon::CONVERT_NONE: break;
      
      case io_cineon::CONVERT_U8:
        info.pixel = pixel::pixel(info.pixel, pixel::U8);
        //info.endian = memory::endian();
        break;
     
      case io_cineon::CONVERT_U16:
